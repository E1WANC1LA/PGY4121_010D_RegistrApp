...
export class BarcodeScanningModalComponent
  ...
  private async startScan(): Promise<void> {
    ...
    // Usa el evento 'barcodesScanned' en lugar de 'barcodeScanned'
    const listener = await BarcodeScanner.addListener(
      'barcodesScanned',
      async (event) => {
        this.ngZone.run(() => {
          const cornerPoints = event.barcodes[0]?.cornerPoints; // Maneja múltiples códigos
          ...
          listener.remove();
          this.closeModal(event.barcodes[0]); // Usa el primer código si hay varios
        });
      }
    );
